...
export function fetchReposByUser(sources) {
  const user$ = sources.ACTION.filter(action => action.type === ActionTypes.REQUESTED_USER_REPOS).map(
    action => action.payload.user
  )

  const request$ = user$.map(user => ({
  ...
  const action$ = xs.combine(response$, user$).map(arr => actions.receiveUserRepos(arr[1], arr[0].body))
...
export function searchUsers(sources) {
  const searchQuery$ = sources.ACTION.filter(action => action.type === ActionTypes.SEARCHED_USERS)
    ...
    .map(q =>
      sources.Time.periodic(800)
        .take(1)
        .mapTo(q)
        .endWhen(sources.ACTION.filter(action => action.type === ActionTypes.CLEARED_SEARCH_RESULTS))
    )
    .flatten()
...
function clearSearchResults(sources) {
  const clear$ = sources.ACTION.filter(action => action.type === ActionTypes.SEARCHED_USERS)
